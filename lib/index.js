...
async function run(file, args, timeoutMs, cwd) {
	try {
...
async function logRun(log, run, file, args, timeoutMs, cwd, label) {
...
async function applyUpdate(opts) {
	...
	const workDiff = await run("git", gitArgs(repoPath, sslBackend, ["diff", "--quiet"]), 1e4);
	if (workDiff.code !== 0 && workDiff.code !== 1) return {
		ok: false,
		code: "git",
		message: "无法读取工作区状态",
		detail: workDiff.stderr.trim()
	};
	const indexDiff = await run("git", gitArgs(repoPath, sslBackend, [
		"diff",
		"--cached",
		"--quiet"
	]), 1e4);
	if (indexDiff.code !== 0 && indexDiff.code !== 1) return {
		ok: false,
		code: "git",
		message: "无法读取暂存区状态",
		detail: indexDiff.stderr.trim()
	};
	...
	if (opts.rebuildAfterUpdate) {
		...
		if (await logRun(log, run, install.file, install.args, opts.rebuildTimeoutMs, repoPath, "pnpm install") === 0) {
			const build = pnpmCommand(["run", "build"]);
			rebuilt = await logRun(log, run, build.file, build.args, opts.rebuildTimeoutMs, repoPath, "pnpm run build") === 0;
